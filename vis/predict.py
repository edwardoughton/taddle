...
CONFIG_COUNTRY.read(os.path.join(os.path.dirname(__file__), '..', 'script_config.ini'))
...
def load_results(metrics, country):
    """
    ...
    consumption = pd.read_csv(os.path.join(RESULTS_DIR, f'ridge_{metrics[0]}', 'predictions.csv'))
    phone_density = pd.read_csv(os.path.join(RESULTS_DIR, f'ridge_{metrics[1]}', 'predictions.csv'))
    phone_consumption = pd.read_csv(os.path.join(RESULTS_DIR, f'ridge_{metrics[2]}', 'predictions.csv'))
